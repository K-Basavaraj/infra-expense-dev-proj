...
#### With VPN:
Laptop becomes part of AWS VPC
Internal Load Balancers and servers are accessible from browser
...
This OpenVPN AMI is public and:
...
The VPN security group allows the following ports:
Port	    Purpose
...
- Should client be allowed to access network services on the VPN gateway IP? YES
Why:
* Allows clients to access internal services
* Required for proper VPC connectivity

#### DNS Settings – Why These Are Important
- Have clients use specific DNS servers as the access server host? NO/Disable
Why:
* Prevents dependency on VPN server DNS
* Avoids DNS resolution issues

- ave clients use specific DNS servers? YES
* Configured DNS:
Primary: 8.8.8.8
Secondary: 8.8.4.4
